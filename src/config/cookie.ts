...
function parseExpiresIn(timeStr: string): number | undefined {
  ...
  switch (unit) {
    case "m":
      return value / (60 * 24); // minutes to days
    case "h":
      return value / 24; // hours to days
    case "d":
      return value; // days
    case "M":
      return value * 30; // months to days (approximate)
    case "y":
      return value * 365; // years to days (approximate)
    default:
      return undefined;
  }
}
...
export function SaveCookie(key: string, state: any, expiresIn: string) {
  try {
    ...
    if (expiresDays === undefined) throw new Error("Invalid expiresIn format");
